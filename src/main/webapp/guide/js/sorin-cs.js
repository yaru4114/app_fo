...
if ($('.tbl-newsletter').length > 0) {
    $('.tbl-newsletter tr').on('mouseover focusin', function(){
        $(this).addClass('hover');
    });
    $('.tbl-newsletter tr').on('mouseleave focusout', function(){
        $(this).removeClass('hover').removeAttr('class');
    });
}
/* 뉴스 클리핑 */
if ($('.tbl-newsclip').length > 0) {
    $('.tbl-newsclip tr').on('mouseover focusin', function(){
        $(this).addClass('hover');
    });
    $('.tbl-newsclip tr').on('mouseleave focusout', function(){
        $(this).removeClass('hover').removeAttr('class');
    });
}
/* 원자재 캘린더 */
if ($('.fullcal-wrap').length > 0) {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        $('.fc-next-button, .fc-prev-button, .fc-dayGridMonth-button, .fc-today-button').on('click', function(){
          replaceDay();
        ...
        $('.fc-listMonth-button, .fc-listDay-button').on('click', function(){
